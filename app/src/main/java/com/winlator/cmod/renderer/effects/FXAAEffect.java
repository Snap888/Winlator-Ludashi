    ...
    private class FXAAMaterial extends ScreenMaterial {
        ...
        protected String getFragmentShader() {
            ...
            return String.join("\n", new CharSequence[]{
                    ...
                    // Используем более чувствительную к цвету константу для яркости
                    "const vec3 luma = vec3(0.299, 0.587, 0.114);", // Стандартный вес для яркости
                    "void main() {",
                    ...
                    // --- Улучшенное определение минимума/максимума для уменьшения дрожания --- ",
                    // Сравниваем центральный пиксель с диагональными и находим мин/макс среди них
                    "    float lumaMin = min(lumaM, min(min(lumaNW, lumaNE), min(lumaSW, lumaSE)));",
                    "    float lumaMax = max(lumaM, max(max(lumaNW, lumaNE), max(lumaSW, lumaSE)));",

                    // Также можно добавить небольшой порог, чтобы избежать срабатывания на шум
                    "    float lumaRange = lumaMax - lumaMin;",
                    "    if (lumaRange < max(0.01, lumaMax * 0.08)) {", // Порог может быть настроен
                    "        gl_FragColor = vec4(rgbM, 1.0);", // Возвращаем оригинальный пиксель, если разница мала
                    "        return;",
                    "    }",
                    // --- Конец улучшения ---",
                    ...
                    // Улучшенное вычисление уменьшения направления
